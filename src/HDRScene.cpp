...
namespace HDR {
	...
	HRESULT initTemporaryResources()
	{
		...
		g_pBackBuffer->GetDesc(&t_Desc);	// this would actually increase the reference count on backbuffer by 1
	...
	{
		g_pBackBuffer->Release();	// This is very weird, the ref count on backbuffer got further increased by one. Therefore, we need to release it twice.
	...
	void setHDRRenderTarget()
	{
		...
		//m_pDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_ARGB(0, 64, 64, 128), 1.0f, 0); // sky blue
